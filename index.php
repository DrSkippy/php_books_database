...
if (substr($_REQUEST['submit'],0,4) == 'Find') {
	...
	$tmp_min_id = $min_id;
		...
		if (!($tmp_min_id < $max_id)) $tmp_min_id = $min_id;
	#	}
	...
	else $recordselector = $row['BookCollectionID'];
	}
# scroll forward or backward
else if ($_REQUEST['submit'] == '   >   ') {
	$recordselector = $_REQUEST['rs'] + 1;
	if ($recordselector > $max_id) $recordselector = $min_id;
	}
else if ($_REQUEST['submit'] == '   <   ') {
	$recordselector = $_REQUEST['rs'] - 1;
	if ($recordselector < $min_id) $recordselector = $max_id;
	}
# Update record with changes
	...
	if ($_REQUEST['recycled'] == 'on') $recycled = 1;
	else $recycled = 0;
	$update_str = 'UPDATE `book collection` SET '.
	 'Title="'.$_REQUEST['title'].'", Author= "'.$_REQUEST['author'].
	 '", CopyrightDate="'.date("Y-m-d h:m:s",$dt).
	 '", ISBNNumber13="'.$_REQUEST['isbnnumber13'].
	 '", ISBNNumber="'.$_REQUEST['isbnnumber'].'", PublisherName= "'.$_REQUEST['publishername'].
	 '", CoverType="'.$_REQUEST['covertype'].'", Pages= "'.$_REQUEST['pages'].
	 '", LastRead="'.$lastread.'",PreviouslyRead= "'.$previouslyread.
	 '", Location="'.$_REQUEST['location'].'", Note="'.$_REQUEST['note'].'", Recycled="'.$recycled.
	 '" WHERE BookCollectionID = '.$_REQUEST['id'];
	if ($debug) echo $update_str;
...
# Add new tag to this record
	...
	$insert_str = 'INSERT into tags (Tag, BookID) VALUES ("'.
		$_REQUEST['newtag'].'","'.$_REQUEST['id'].'")';

	if ($debug) echo $insert_str;
	if ( $_REQUEST['newtag'] !='') {
		$result = @mysql_query($insert_str);
		if (!$result) {
			echo('<p>'.$insert_str.'Error performing query: '.mysql_error().'</p>');
			exit();
			}
		}
	}
...
if ($row['BookCollectionID'] <> '') {
	...
	if ($_REQUEST['submit'] == '   >   ') $recordselector = $recordselector + 1;
	else $recordselector = $recordselector - 1;
	}
}
# Once we have found a valid record, populate the tags string for display
$tagsearch_str = 'SELECT DISTINCT Tag FROM tags WHERE BookID = '.
	$row['BookCollectionID'];
if ($debug) echo $tagsearch_str;
$tagresult = @mysql_query($tagsearch_str);
if (! $tagresult) {
	 exit('<p>'.$tagsearch_str.'Error performing query: '.mysql_error().'</p>');
       }
if ($tagrows = mysql_fetch_array($tagresult)){
	$itemtags = $tagrows['Tag'];
	while ($tagrows = mysql_fetch_array($tagresult)){
...
<?php echo ('<input type="hidden" name="rs" value="'.$recordselector.'">');?>
<table>
<tr>
	<td align="right" width="100">Author:</td>
	<td><input type="text" size="25" name="author" value="<?php echo $row['Author']?>"></td>
	<td align="right" width="110">Title:</td>
	<td><input type="text" size="30" name="title" value="<?php echo $row['Title']?>"></td>
</tr><tr>
	<td align="right">ISBN #:</td>
	<td><input type="text" size="15" name="isbnnumber" value="<?php echo $row['ISBNNumber']?>"></td>
	<td align="right">ISBN13 #:</td>
	<td><input type="text" size="15" name="isbnnumber13" value="<?php echo $row['ISBNNumber13']?>"></td>
</tr><tr>
	<td align="right">Publisher:</td>
	<td><input type="text" size="25" name="publishername" value="<?php echo $row['PublisherName']?>"></td>
	<td align="right"> </td>
	<td> </td>
</tr><tr>
	<td align="right" valign="top">Note:</td>
	<td colspan="2"><textarea name="note" cols="28" rows="3"><?php echo $row['Note'];?></textarea>
	</td>
	<td><input type="radio" name="covertype" value="Hard"<?php if (substr($row['CoverType'],0,4) == 'Hard') echo'checked';?>
	> Hard Cover</br>
	    <input type="radio" name="covertype" value="Soft"<?php if (substr($row['CoverType'],0,4) == 'Soft') echo'checked';?>
	> Soft Cover</br>
		<input type="radio" name="covertype" value="Digital"<?php if (substr($row['CoverType'],0,4) == 'Digi') echo'checked';?>
	> Digital</br>
	Recycled: <input type="checkbox" name="recycled"
<?php if ($row['Recycled']) echo 'checked'?>></input>
	</td>
</tr><tr>
...
$result1 = @mysql_query('SELECT DISTINCT Location FROM `book collection` ORDER BY Location');
if (!$result1) {
...
?>
	...
	<td><input type="text" name="pages" size="6" value="<?php echo $row['Pages']?>"></td>
</tr><tr>
	<td align="right">Copyright:</td>
	<td><input type="text" name="copyrightdate" size="5" value="<?php echo substr($row['CopyrightDate'],0,4)?>"></td>
	<td></td>
...
?>"></input></td>
	...
	<td colspan="4"><input type="text" name="newtag" size="6">&nbsp; <input type="submit" name="submit" value="Add Tag"> Tags: <span class="tags"> <?php echo $itemtags ?></span></td>
</tr></table>
<table>
<tr>
	<td align="left">
	...
	echo $row['ISBNNumber'];?>/" target="_blank">See it @ Amazon</a></td>
	<td>ID:<?php echo $row['BookCollectionID'];?>
	<input type="hidden" name="id" value="<?php echo $row['BookCollectionID'];?>">
	&nbsp;&nbsp;&nbsp;&nbsp;<a href="./add.php">Add Entry</a></td>
</tr><tr>
	<td align="center">
		<input type="text" name="searchterm" size="25" value="<?php echo $sterm;?>">
	<input type="submit" name="submit" value="Find">
<!--	<input type="submit" name="submit" value="Find Again"> -->
	<input type="radio" name="searchtype" value="Title"
<?php if($_REQUEST['searchtype'] == "Title") echo 'checked';
	echo '>Title&nbsp;&nbsp;<input type="radio" name="searchtype" value="Author"';
	if($_REQUEST['searchtype'] <> "Title") echo 'checked'; ?>
>Author</td>
</tr>
</table>
</form>
...
<tr>
	...
	<?php include('./location_option.php'); ?>
	</select></td>
</tr><tr>
