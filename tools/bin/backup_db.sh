#!/usr/bin/env bash
# Backup the book_service database
...
#
if [ -d $DEST ];  then
  mysqldump --single-transaction --column-statistics=0 -h$DBHOST -u$UN -p$PWD $DB > $DEST/$BOOKSDBDUMP
